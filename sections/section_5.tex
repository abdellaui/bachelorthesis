...
In dieser Arbeit wurden die realen Evaluationsdaten mit einer konstruierten Hardware erhoben. Diese versprach bei Bestkonditionen eine Abweichung (\textit{Drift}) von weniger als 1\%. Aus Zeitgründen konnten die Bestkonditionen für die Hardware nicht hergestellt werden, sodass die berechneten Posen einen Drift bis zu 5\% (vgl. Abb. \ref{fig:trajectories}) aufzeigten. 

Die Hardware berechnet die Pose über IMU-gestütztes vSLAM Algorithmen. Angesichts des SLAM Algorithmuses kann durch ein \textit{Loop-Closure} der Drift klein gehalten werden. Ebenso kann der Drift durch das langsame Fortbewegung der Hardware entgegengewirkt werden, da der vSLAM Algorithmus mehr Zeit sowie visuellen Input für die Bestimmung der Pose erhält.
