...
Weiterhin haben die Ergebnisse der Strecken \textit{IC-loop} und \textit{HS-gamma} gezeigt, dass die Evaluationsdaten auf eine ca. 5$m$ breiten und ca. 20$m$ bis 30$m$ langen Teilzone verteilt werden. In \textit{IC-loop} kann das Lokalisieren auf einem begrenzten Gebäudeareal auf perceptual-aliasing zurückgeführt werden, da sich die Innenräume der vertikal sowie horizontal verlaufende Strecken stark ähnelten. Im Gegensatz dazu waren in \textit{HS-gamma} die Innenräume der vertikal verlaufenden und entlang der Schleife verlaufenden Strecken von den horizontal verlaufenden  Strecken differenzierbar.  Dennoch werden in \textit{HS-gamma} die Evaluationsdaten \textit{nur} auf einen Bereich der horizontalen Strecke lokalisiert. Das Lokalisieren auf nur einen Bereich der horizontalen Strecke kann auf perceptual-aliasing
