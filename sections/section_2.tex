...
% - rgb-d
% - image only
\textbf{Pose Estimation} ist eine Methode der visuellen Lokalisierung \textit{(Visual-Based Localization, kurz VBL)}. VBL beschäftigt sich mit der Bestimmung der Pose (Position + Orientierung) eines visuellen Abfragematerials (z.B. ein RGB-Bild) in einer zuvor bekannten Szene  \cite{piascoSurveyVisualBasedLocalization2018}.
Ein nah liegendes Themengebiet der Robotik ist die visuelle Ortswiedererkennung (Visual Place Recognition, kurz VPR) \cite{lowryVisualPlaceRecognition2016}. Die visuelle Ortswiedererkennung fokussiert sich auf das Feststellen eines bereits besuchten Ortes und definiert sich aus den drei Datenverarbeitungs-, Kartografierungs- sowie Orientierungsmodulen. Allgemein lässt sich das Prozess eines VPRs \cite{lowryVisualPlaceRecognition2016} folgend beschreiben.  Das Datenverarbeitungsmodul bereitet die Daten entsprechend vor. Das Kartografiemodul erstellt eine interne Karte besuchter Orte. Dabei entscheidet das Orientierungsmodul, auf der Grundlage des aktuellen Inputs und der immer aktuell gehaltenen Karte, ob ein Ort bereits besucht wurde. Im Vergleich zur VPR \cite{lowryVisualPlaceRecognition2016} versucht die visuellen Lokalisierung \cite{piascoSurveyVisualBasedLocalization2018} eine Pose zu bestimmen und bedarf daher keiner Kartographierungsmodul.
