...
Künstliche Neuronen können zu einer Schicht (\textit{layer}) zusammengeführt werden. Verbindungen solcher Schichten bilden einen neuronales Netzwerk. Diese Arbeit beschränkt sich einfachhaltshalber nur auf die fully-connected Verbindungsart der Schichten.
Bei einem \textit{feedforward} Netzwerk übergibt jedes Neuron aus der Schicht $l$ seinen Output $y_{l}$ an die Neuronen der Schicht $l+1$ weiter. Ebenso sind Neuronen aus der gleichen Schicht untereinander nicht verbunden \cite{Goodfellow-et-al-2016}. 
...
Einfache neuronale Netze, wie sie in Abschnitt \ref{sec:KNN} beschrieben wurden, arbeitet auf ein Inputsvektor $x \in \mathbb{R}^{M}.

CNNs haben eine ähnliche Struktur zu dem einfacheh beschriebene Netzwerk in Abschnitt \ref{sec:KNN} 
onvolutional Neural Networks are very similar to ordinary Neural Networks from the previous chapter:
