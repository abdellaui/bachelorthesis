...
Ist jedes Neuron aus der Schicht $l$ mit jedem Neuron aus der Schicht $l+1$ verbunden, so ist die Rede von einem \textit{fully-connected} Netzwerk  \cite{CS231nConvolutionalNeural}. Im weiteren Verlauf dieses Kapitels wird einfachheitshalber bei einem Netzwerk immer ein fully-connected Netz angenommen.
Bei einem \textit{feedforward} Netzwerk übergibt jedes Neuron aus der Schicht $l$ seinen Output $y_{l}$ nur an die Neuronen der Schicht $l+1$ weiter. Ebenso sind Neuronen aus der gleichen Schicht untereinander nicht verbunden \cite{Goodfellow-et-al-2016}.
...
Abilldung \ref{fig:neural_net} illustriert ein fully-connected feedforward neuronales Netz als ein azyklischer Graph.
...
\begin{figure}[hbt]
	...
	\caption{Ein fully-connected feedforward neuronales Netz mit der Tiefe 3 bestehend aus einem Input-Layer der Breite 3, aus zwei Hidden-Layer der Breite 4 und einem Output-Layer der Breite 1. Mit der Gleichung \ref{eq:network}  lässt sich dieses Netzwerk als die Funktion $f :  \mathbb{R}^{3} \mapsto \mathbb{R}^{1}$ mit $f(x)=f_3(f_2(f_1(x)))$ darstellen  }
	\label{fig:neural_net}
\end{figure}

\subsection{Convolutional Neural Networks}

Einfache neuronale Netze, wie sie in Abschnitt \ref{sec:KNN} beschrieben wurden, arbeitet auf einem Inputsvektor $x \in \mathbb{R}^{M}$.
