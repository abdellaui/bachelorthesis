...
Es existiert eine Menge von bekannten CNN Modellen mit ausgezeichneten Ergebnissen in internationalen Wettbewerben, solche wie die ILSVRC \cite{russakovskyImageNetLargeScale2014}. Dieses Kapitel behandelt nur die Architektur des GoogLeNet Modells und dessen Modifikation PoseNet.

\subsubsection{GoogLeNet}
\label{sec:googlenet}
GoogLeNet, konstruiert von \citet{szegedyGoingDeeperConvolutions2015} im Jahr 2014, ist der Sieger des ILSVRC14 Wettbewerbes. Das GoogLeNet Modell kennzeichnet sich durch.

Inzwischen wurde das Inception Modul \cite{szegedyRethinkingInceptionArchitecture2016} und die gesamte Architektur herum \cite{szegedyInceptionv4InceptionResNetImpact2016} weiterentwickelt.
...
Abbildung \ref{fig:inception_module} stellt das Inception (\textit{v1}) Modul dar. Abbildung \ref{fig:googlenet} illustriert die GoogLeNet Architektur in Detail.
 ...
 \begin{figure}[H]
	...
	\caption{Darstelltung des Inception (\textit{v1}) Moduls. Das Inputvolumen wird auf 4 Zweige unabhängig verarbeitet. Die Ergebnisse der unabhängigen Operationen haben die gleiche Breite und Höhe. Der Output des Inception Moduls wird durch das Konkatenieren (in der Tiefe) der einzelnen Ergebnisse bestimmt. Entnommen aus \cite{szegedyGoingDeeperConvolutions2015}.}
