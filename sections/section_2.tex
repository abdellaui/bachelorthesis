...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{images/neural_net.png}
...
Ein Inception Modul beinhaltet dabei mehrere Layer und verarbeitet das Inputvolumen parallel auf 4 Zweigen. Vor rechenintensiven Convolutional Layer werden zusätzliche 1$\times$1 dimensionsreduzierende Layer geschaltet. Abschließend werden die Zweige, welche ein Volumen mit der gleiche Breite und Höhe produzieren, zu einem Outputvolumen (in die Tiefe) zusammengeführt.  Durch die parallele Berechnung der Zweige und das Einschalten von dimensionsreduzierenden 1$\times$1 Layer vor rechenintensiven Operationen, gelingt es den Autoren ein tiefes und breites neuronales Reche

Inzwischen wurde das Inception Modul \cite{szegedyRethinkingInceptionArchitecture2016} und die gesamte Architektur rundherum \cite{szegedyInceptionv4InceptionResNetImpact2016} weiterentwickelt.
 
Abbildung \ref{fig:inception_module} stellt das Inception (\textit{v1}) Modul dar. Abbildung \ref{fig:googlenet} illustriert die GoogLeNet Architektur in Detail.

 \begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{images/googlenet/inception_module.png}
	\caption{Darstelltung des Inception (\textit{v1}) Moduls. Das Inputvolumen wird auf 4 Zweige unabhängig verarbeitet. Die Ergebnisse der unabhängigen Operationen haben die gleiche Breite und Höhe. Der Output des Inception Moduls wird durch das Konkatenieren (in der Tiefe) der einzelnen Ergebnisse bestimmt. Die gelben 1$\times$1 Convolution Layer dienen zur Dimensionsreduktion. Entnommen aus \cite{szegedyGoingDeeperConvolutions2015}.}
