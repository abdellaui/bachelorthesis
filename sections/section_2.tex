...
Dosovitskiy \etal\cite{dosovitskiyFlowNetLearningOptical2015} erlernen mit synthetischen Daten den optischen Fluss von Bildsequenzen.  Auf Hintergrundbildern aus einer Sammlung werden mehrmals bewegte virtuelle Stühle platziert.

Getrieben vom Einsatz von simulierten 3D-Daten zur Datengenerierung stellt Ha \etal\cite{haImagebasedIndoorLocalization2018} einen Ansatz zur Bild-basierte Lokalisierung in Gebäuden vor. Dieser Ansatz generiert synthetische Daten aus einem Building Information Modeling \textit{(BIM)}. Es sieht in dem künstlichen Datensatz die durch ein CNN extrahierte Features als wesentlich und legt diese in einer Datenbank ab. Ein reales Aufnahmebild im Gebäude lässt sich durch den Vergleich der Features
lokalisieren.
