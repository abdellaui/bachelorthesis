...
Alle Datensätze beinhalten Herausforderungen wie z.B. das \textit{Loop-Closure}- oder \textit{Perceptual-Aliasing}-Problem. Die ca. $115m$ lange Strecke im ersten Datensatz, bezeichnet als \textit{IC-Loop}, bildet eine geschlossene Schleife in einem $50m \times 11m \times 3.5m$ Bereich der Gebäude a). Die $65m$ lange Strecke im \textit{HS-gamma} Datensatz einhält in der Mitte eine Schleife. Der Anfangs- und Endpunkte der Strecke im \textit{HS-gamma} Datensatz sind in optisch ähnlichen gegenüberliegenden Punkten der Gebäude b) in einem Bereich von ca. $54m \times 10m \times 3m$. Im Datensatz \textit{HS-stairs-up} wird eine Treppe der Gebäude b) aufwärts bestiegen und im Datensatz \textit{HS-stairs-down} dieselbe Treppe abgestiegen. Die Strecken auf den Treppen sind  ca. $32m$ lang und befindet sich in einer ca. $12m \times 12m \times 12m$ Teilzone der Gebäude b). 
...
\begin{figure}[htp]
	...
	\end{subfigure}
	\hfill
	\caption{Illustration der Aufnahmestrecken pro Datensatz. \ref{subfig:traj_ic} befindet sich in der nördlichen Hälfte des 6. Stockwerkes des IC-Gebäudes Ruhr-Universität Bochum. \ref{subfig:traj_hs_gamma}, \ref{subfig:traj_hs-up}, \ref{subfig:traj_hs-down} befinden sich im Seminargebäude Hochschule Bochum. Die Tabelle \ref{tab:dataset_metrics} listet die approximierten metrischen Eigenschaften der Aufnahmenstrecken auf.}
