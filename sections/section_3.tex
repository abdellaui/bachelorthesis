...
Die Leistung eines künstlichen neuronalen Netzwerkes ist abhängig von ihren Parametern. Es gibt zwei Arten der Parameter. Die erste Art von Parametern (\textit{Gewichte}) werden während der Trainingsphase an die Trainingsdaten angepasst. \textit{Underfitting} entsteht, wenn das Netzwerk anhand der Trainingsdaten unfähig ist unbekannte Daten zu generalisieren. \textit{Overfitting} entsteht, wenn das Netzwerk die Trainingsdaten auswendig lernt. Bei der Anpassung der Gewichte wird die Fähigkeit der Generalisierung bei unbekannten Daten über das Erlenen durch die Trainingsdaten gestrebt. Die zweite Art von Parametern (\textit{Hyperparameter}) werden vor der Trainingsphase festgelegt und sind konstant über den Trainingsprozess hinweg. Hyperparameter geben besondere Einstellungen des Netzwerkes an wie z.B. das Lernverhalten und können über Grid-Search Verfahren optimiert werden \cite{Goodfellow-et-al-2016}.
