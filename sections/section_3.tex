...
{fig:treshold}). Die Pixelwerte der Artefakten befinden sich im niedrigen Wertebereich. Für die Unterdrückung der Artefakten wird zusätzlich ein Schwellenwertverfahren angewendet, indem die Pixelwerte eines Gradientenbildes auf ein Mindestwert gesetzt werden. Es wird iterativ nach einem Wert gesucht, der die Pixelwerte des Gradientenbildes so anhebt, sodass die die Artefakte unterdrückt werden. Der Wert 8 stellt sich als geeignete Schwelle für die Gradientenbilder im Wertebereich von $[0,255]$ heraus. Abbildung \ref{fig:dataset_preprocess} visualisiert von jedem Datensatztyp ein Beispiel und die dazugehörigen Gradientenbilder.
...
\begin{figure}[H]
	...
	\zoombox[magnification=10]{0.5,0.25}
	\end{tikzpicture}
	\caption{Darstellung der durch die künstliche Beleuchtung entstehenden Artefakte in den Gradientenbilder. \subref{gradma-image} ist ein unveränderter Gradientenbild. Für eine bessere Visualisierung der Artefakte wird in \subref{tbinared-image} das Gradientenbild \subref{gradma-image}  binarisiert. }
	\label{fig:treshold}
\end{figure}
