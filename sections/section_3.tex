...
{fig:treshold}). Die Pixelwerte der Artefakten befinden sich im niedrigen Wertebereich. Für die Unterdrückung der Artefakten wird zusätzlich ein Schwellenwertverfahren angewendet, indem die Pixelwerte eines Gradientenbildes auf ein Mindestwert gesetzt werden. Es wird iterativ nach einem Wert gesucht, der die Pixelwerte des Gradientenbildes so anhebt, sodass die Artefakte unterdrückt werden. Der Wert 8 stellt sich als geeignete Schwelle für die Gradientenbilder im Wertebereich von $[0,255]$ heraus. Abbildung \ref{fig:dataset_preprocess} visualisiert von jedem Datensatztyp ein Beispiel und die dazugehörigen Gradientenbilder.
