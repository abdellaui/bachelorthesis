...
Künstliche neuronale Netze bzw. CNNs werden zuerst modelliert und anschließend mit entsprechenden Daten trainiert. Die vorliegende Arbeit verwendet die Caffe \cite{jiaCaffeConvolutionalArchitecture2014} Implementierung der PoseNet Architektur, die in Abschnitt \ref{sec:posenet} beschrieben wird. Die Gewichte des Netzwerks wurde mit den Gewichten eines auf der GoogLeNet Architektur mit dem Places Datensatz \cite{zhouLearningDeepFeatures2014} trainierten Models initialisiert. 

% Die Initialisierung der Gewichte mit den trainierten Gewichten eines Models wird in der Litaratur als \textit{transfer learning} bezeichnet. Transfer learning ermöglicht mit
...
Die Gebäude wurden in Blender\footnote{\url{https://www.blender.org} (aufgerufen am: 20.07.2019)} Version 2.79b simuliert. Die Strecke der echten Aufnahmen wurden in den Simulationen schwankungslos auf einer konstanten Höhe von 1.70$m$ imitiert.


Die intrinsischen Daten der D435 RGB-Kamera wurden auf die virtuellen Kameras übertragen. Es wurde entlang der Strecke in 0.05$m$ Intervallen und mit einer $\pm$10° Neigung in je y- und z-Achse Bilder (siehe Abbildung \ref{fig:dataset_variation}) mit korrespondierenden Ground-Truth-Daten aufgenommen.
