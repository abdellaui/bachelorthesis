...
Für alle synthetischen Datensätze wurde das Netzwerk separat 5-mal mit den korrespondierenden Gradientenbilder trainiert. Eine Evaluierung folgte mit den Gradientenbilder der korrespondierenden synthetischen sowie realen Evaluationsdaten. Es wurden je Strecke (s. Tabelle \ref{tab:dataset_metrics})  nur die bessere Akkuratesse behalten. Tabelle \ref{tab:results_ic} bis \ref{tab:results_hs_stairs_down} geben die Akkuratesse des KNN auf der jeweiligen Strecken an. Im weiteren Verlauf dieses Kapitels ist einfachheitshalber bei einem Datensatz immer die Gradientenbildern des jeweiligen Datensatzes gemeint.
...
In diesem Experiment wurden alle Datensätze zu 50\% Trainingsdaten und zu 50\% Evalierungsdaten zufällig aufgeteilt. Es wurde für alle synthetischen Datensatztypen der Strecke \textit{IC-loop} mit je 5718 Daten ein Netzwerk trainiert. Anschließend wurden die Netzwerke mit den korrespondierenden synthetischen Evaluierungsdatensätze mit je 5717 Daten getestet. Eine weitere Evaluierung folgte mit den realen Evaluierungsdaten mit 1921 Daten.
