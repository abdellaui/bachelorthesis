...
\begin{table}
	\centering
	\caption{Referenzwerte der Akkuratesse. Diese Werte wurden während der Bestimmung des $\beta$ Hyperparameters durch das Trainieren und Evaluieren mit den realen Daten ermittelt}
	\begin{tabularx}{0.7\textwidth}{X >{\RaggedRight}X}
		\textbf{Strecke} & \textbf{Akkuratesse} \hspace{2cm} (Position, Orientierung)\\
		\hline
		\textit{IC-loop} & 1.93$m$, 4.26°\\
		\hline
		\textit{HS-gamma} & 0.95$m$, 7.53°\\
		\hline
		\textit{HS-stairs-up} & 0.94$m$, 8.33°\\
		\hline
		\textit{HS-stairs-down} & 0.87$m$, 9.25°\\
		\hhline{|=|=|}
		Durchschnitt & 1.17$m$, 9.79°\\
	\end{tabularx}
