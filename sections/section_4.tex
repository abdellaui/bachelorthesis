...
\subsection{Evaluation des Netzwerkes}

\begin{table}
	\centering
	\caption{Evaluationsergebnisse von IC-loop}
	\begin{tabularx}{1.0\textwidth}{>{\RaggedRight}X >{\RaggedRight}X >{\RaggedRight}X}\textbf{Datensatztyp} \hspace{2cm} (Gradientenbild) & \textbf{synthetische Daten} \hspace{1cm} (Position, Orientierung) & \textbf{reale Daten} \hspace{2cm} (Position, Orientierung)\\
		\hline
...
\begin{table}
	...
	\begin{tabularx}{1.0\textwidth}{>{\RaggedRight}X >{\RaggedRight}X >{\RaggedRight}X}\textbf{Datensatztyp} \hspace{2cm} (Gradientenbild) & \textbf{synthetische Daten} \hspace{1cm} (Position, Orientierung) & \textbf{reale Daten} \hspace{2cm} (Position, Orientierung)\\
	\hline
		karikatur. Simulation & 1.00$m$, 9.92° & 8.60$m$, 19.59°\\
		\hline
		synthe. Kantenbild & 1.07$m$, 8.69° & 10.15$m$, 35.11°\\
		\hline
		fotoreal. Simulation & 1.45$m$, 9.17° & 10.27$m$, 41.60°\\
	\end{tabularx}
	\label{tab:res_hs_gamma}
\end{table}
...
\begin{table}
	...
	\begin{tabularx}{1.0\textwidth}{>{\RaggedRight}X >{\RaggedRight}X >{\RaggedRight}X}\textbf{Datensatztyp} \hspace{2cm} (Gradientenbild) & \textbf{synthetische Daten} \hspace{1cm} (Position, Orientierung) & \textbf{reale Daten} \hspace{2cm} (Position, Orientierung)\\
	\hline
		karikatur. Simulation & 0.82$m$, 7.76° & 4.77$m$, 23.43°\\
		\hline
		synthe. Kantenbild & 0.82$m$, 8.48° & 4.33$m$, 51.64°\\
		\hline
		fotoreal. Simulation & 0.92$m$, 7.98° & 5.16$m$, 93.38°\\
	\end{tabularx}
	\label{tab:res_hs_stairs_up}
\end{table}
...
\begin{table}
	...
	\begin{tabularx}{1.0\textwidth}{>{\RaggedRight}X >{\RaggedRight}X >{\RaggedRight}X}\textbf{Datensatztyp} \hspace{2cm} (Gradientenbild) & \textbf{synthetische Daten} \hspace{1cm} (Position, Orientierung) & \textbf{reale Daten} \hspace{2cm} (Position, Orientierung)\\
	\hline
		karikatur. Simulation & 0.91$m$, 8.01° & 4.20$m$, 47.83°\\
		\hline
		synthe. Kantenbild & 0.85$m$, 7.50° & 5.59$m$, 67.34°\\
		\hline
		fotoreal. Simulation & 1.02$m$, 8.57° & 5.25$m$, 32.70°\\
	\end{tabularx}
	\label{tab:res_hs_stairs_down}
\end{table}
