...
Das mit \textit{grad-cartoon} trainierte Netzwerk bestimmte die Positionen aller \textit{grad-real} Evaluationsdaten nahe der erwähnten Schleife in der \textit{HS-gamma} Strecke auf ein ca. 20$m$ langem Teilbereich (vgl. Abb.  \ref{subfig:hs_gamma_fig2}). Deshalb weisen die Evaluationsdaten nahe der Schleife die geringsten Positionsfehler auf. Die größten Positionsfehlern zeigen die Evaluationsdaten des zum Ausgangspunkt optisch ähnlichen Flures (vgl. Abb. \ref{subfig:ic_fig4}). Zusätzlich ist in Abbildung \ref{subfig:ic_fig6} zu erkennen, dass das Netzwerk größtenteils die Orientierung der Evaluationsdaten als die dominierende Aufnahmerichtung bestimmte. Daher sind die größten Orientierungsfehler  Offensichtlich war das Netzwerk unfähig zur Pose Estimation in sich optisch ähnelnden Strecken.
