...
Das mit \textit{grad-cartoon} trainierte Netzwerk bestimmte 
In Abbildung \ref{subfig:hs_gamma_fig2} ist deutlich zu erkennen, dass das mit \textit{grad-cartoon} trainierte Netzwerk die Position aller \textit{grad-real} Evaluationsdaten verteilt auf ein ca. 20$m$ langem Teilbereich nahe der Schleife bestimmt hat. Daher weisen Evaluationsdaten der kürzeren vertikalen sowie der obigen horizontalen Strecke die größten Positionsfehler auf (vgl. Abb\ref{subfig:ic_fig4}). Ebenso kommt in Abbildung \ref{subfig:ic_fig6} hervor, dass das Netzwerk größtenteils die Orientierung der Evaluationsdaten als die Aufnahmerichtung der unteren horizontalen Strecke bestimmt hat. Offensichtlich war das Netzwerk unfähig zur Pose Estimation in sich optisch ähnelnden Strecken.
