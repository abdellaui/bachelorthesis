...
\myabstract{The ability of indoor pose estimation is an essential key component for a wide range of applications in construction like progress monitoring of finished work or augmented reality-based indoor facility maintenance and navigation. There are numerous indoor localization methods based on different technologies, where an image-based approach over a mobile device represents the most economical one. This work aimed to look into an existing image-based approach using convolutional neural networks trained on synthetic images to estimate the pose of real images. Four datasets of indoor scenes from two buildings with various trajectory properties were created and independently analyzed. The average position accuracy was 11, while an accuracy about 1m was achievable by training and testing the artificial neural networks with the same domain of data. Furthermore, the networks localized all test data in a confined area and estimated the orientation mostly as the same one learned.}
...
\includeonly{sections/section_1, sections/section_6 }
\begin{document}
